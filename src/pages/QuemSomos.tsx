    ...
    <LegalLayout title="Quem Somos">
      <div className="text-center mb-8">
        <p className="text-lg leading-relaxed text-gray-600">
          O <strong className="text-gray-900">WoomChat</strong> nasceu da vontade de criar um espaço online onde pessoas possam se conectar de forma genuína, segura e divertida.
          Acreditamos que todo mundo merece encontrar alguém especial.
        </p>
      </div>

      <h2><Target size={18} className="text-emerald-600 shrink-0" /> Nossa Missão</h2>
      <div className="bg-emerald-50 rounded-xl p-5 mt-3 border border-emerald-100 text-center">
        <p className="text-sm text-emerald-800 font-medium">Conectar corações por meio de conversas autênticas. O WoomChat é mais do que um bate-papo — é um ambiente onde <strong>relacionamentos reais</strong> começam.</p>
      </div>

      <h2><Star size={18} className="text-emerald-600 shrink-0" /> Nossos Valores</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
        {[
          ...
          { emoji: "🌍", title: "Inclusão", desc: "O WoomChat é para todos, independente de orientação, idade ou localização" },
      ...
      <h2><Sparkles size={18} className="text-emerald-600 shrink-0" /> Como Funciona</h2>
      <div className="flex flex-col sm:flex-row gap-3 mt-4">
        {[
          ...
          <div key={item.step} className="flex-1 bg-white rounded-xl p-4 border border-gray-200 shadow-sm text-center">
            <div className="w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 flex items-center justify-center text-sm font-bold mx-auto mb-2">{item.step}</div>
      ...
      <h2><Users size={18} className="text-emerald-600 shrink-0" /> Nossa Equipe</h2>
      ...
      <div className="mt-10 bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-2xl p-6 text-center shadow-lg">
        ...
        <p className="text-white font-semibold text-lg">WoomChat — Onde corações se encontram.</p>
      </div>
    </LegalLayout>
  );
}
